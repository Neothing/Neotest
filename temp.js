...
setInterval(() => {
  ...
  fs.readdirSync("./temp/media/rtmp3").map((a) => {
    if (a == `media`) {
      return;
    } else {
      console.log(a);
      fs.unlinkSync(`./temp/media/rtmp3/${a}`);
    }
  });
}, 1000 * 60 * 60);
    ...
    if (type == "rtmp3") {
      try {
        ytdl(url, {
          ...
          .pipe(fs.createWriteStream(`./temp/media/${type}/${idn}.mp3`))
          .on("finish", () => {
            res.status(200);
            try {
              res.download(`./temp/media/${type}/${idn}.mp3`, {
                root: __dirname,
              });
            } catch (error) {
              console.error(error);
      ...
      try {
        ytdl(url, {
          ...
          .pipe(fs.createWriteStream(`./temp/media/${type}/${idn}.mp4`))
          .on("finish", () => {
            res.status(200);
            try {
              res.download(`./temp/media/${type}/${idn}.mp4`, {
                root: __dirname,
              });
            } catch (error) {
              console.error(error);
...
app.listen(4500, () => {
  console.log(`[SYSTEM] RTube is Running..!`);
});
